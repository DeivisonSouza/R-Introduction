...
<bdi style="font-weight:bold">Mogno</bdi> = <bdi style="color:green;">*Swietenia macrophylla*</bdi>; e

<bdi style="font-weight:bold">Angelim</bdi> = <bdi style="color:green;">*Hymenolobium petraeum*</bdi>.
]
...
dt <- data.frame(
  Especie = c("Mogno", "Mogno", "Mogno", "Angelim","Angelim",
              "Angelim", "Angelim","Angelim", "Mogno", "Mogno"),
              DAP = c(33.6, 42.6, 52.1, 80.3, 90.8,
                      60.4, 70.5, 100.5, 60.7, 27.1),
              H = c(9.1, 13.2, 15.4, 18.6, 19.5,
                    16.8, 17.9, 22.4, 15.3, 8.7),
              Cipo = c("Sim", "Não", "Sim", "Não", "Sim",
                         "Não", "Sim", "Não", "Sim", "Sim"),
              QF = c(2, 1, 3, 1, 2, 1, 2, 1, 1, 3),
  stringsAsFactors = T)
...
Use o operador lógico `&` para encontrar responder as questões a seguir:

+ Quais árvores possuem <bdi style="font-weight:bold">DAP maior do que 50cm</bdi> `e` <bdi style="font-weight:bold">qualidade de fuste (QF) igual a 1</bdi>?
...
+ Quais árvores não possuem <bdi style="font-weight:bold">cipós presentes</bdi>, <bdi style="font-weight:bold">qualidade de fuste (QF) igual a 1</bdi> `e` <bdi style="font-weight:bold">DAP maior do que 50cm</bdi>?

```{r operE2, echo=TRUE, eval=FALSE}
dt$Cipo == "Não" & dt$QF == 1 & dt$DAP >= 50
```
...
---
+ Quais árvores possuem <bdi style="font-weight:bold">DAP maior do que 50cm</bdi> `e` <bdi style="font-weight:bold">podem ser exploradas</bdi> (cortadas)?

+ Baseado no Diâmetro Mínimo de Corte (DMC) estabelecido no art. 6º da [Resolução CONAMA nº 406 de 02/02/2009](http://www2.mma.gov.br/port/conama/legiabre.cfm?codlegi=597) (Publicado no DOU nº 26, de 06/02/2009, pág. 100) quais árvores são passíveis de corte (exploração) legal (considerando a inexistência de outros impedimentos!)?
